...
userint33  proc  far
  ...
  mov bx , 40 ;початкова верхня границя
  mov ax , 2  ;початкова нижня границя
  mov cx , max_prg  ; к-сть 

base_loop: 
  cmp dx , bx
  ja  inc_loop    ;перевіряємо вихід за верхню границю
  cmp dx , ax
  jl  inc_loop   ;за нижню 
  jmp process 
inc_loop:       
  add bx , 40  
  add ax , 40
...
process:
  ...
  cmp bx , nprg     ; чи є активним
  je exit   ;якщо так , то вихід
  mov init[bx] , ready   ; встнновлюємо значення 
exit:
  popa 
  pop es
  pop ds  
...
b10:
 		mov  	screen_addr[esi*2],bx       	; заповнення таблиці
 	 	...
 	 	mov 	init[si],stop 		; першопочаткове заповнення
                 	                                            		; таблиці стану задач
 		add  	bx,80
                	inc   	si
                	loop 	b10
